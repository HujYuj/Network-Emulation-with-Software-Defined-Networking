...
class RawSocketListener():
    ...
    def get_blacklist(self):
        with open("/home/mininet/cs456-a3/part-D/blacklist.txt", "r") as f:
            lines = f.read()
        blacklist = lines.split(",")
...
def l2_handler(message, middlebox_port, iface_name, blacklist):
    ...
    if eth_frame.haslayer(UDP) and eth_frame[UDP].dport == middlebox_port\
        ...
        # if message contains word from blacklist, simply return and drop the packet
        for word in blacklist:
            if word in payload.decode("utf-8"):
                print(f"Message {payload} received contains word {word} from blacklist, drop the packet.")
                return
        eth_frame[IP].ttl -= 1
        ...
        eth_frame.show2()
        print(f"Verified the UDP payload. Sending the packet: "
