...
def install_udp_middlebox_flow( source_dpid
    ...
    path_client_to_server = get_shortest_path_between(g,source_dpid, destination_dpid)
    path_client_to_middlebox = get_shortest_path_between(g,source_dpid, middlebox_dpid)
    path_middlebox_to_server = get_shortest_path_between(g, middlebox_dpid, destination_dpid)
    # get the client ip and server ip from dpid
    source_ip = get_host_ip_from_dpid(source_dpid)
    destination_ip = get_host_ip_from_dpid(destination_dpid)
    source_mac = get_host_mac_from_dpid(source_dpid)
    destination_mac = get_host_mac_from_dpid(destination_dpid)
    
    # install flows on the path from client to middlebox
    for switch in path_client_to_middlebox:
        # match rules based on 4 pc information, ips and ports
        in_port = get_input_port(path_client_to_middlebox, switch)
        match = build_match_for(source_ip=source_ip, destination_ip=destination_ip, source_port=source_port, destination_port=destination_port, in_port=in_port, source_mac=source_mac, destination_mac=destination_mac)
        switch_index = path_client_to_middlebox.index(switch)
        # if switch is the last one on the path, output port = 1 to the only host
        if switch_index == len(path_client_to_middlebox)-1:
            output_port = 1
        else:
            aj_switch = path_client_to_middlebox[switch_index+1]
            pair_port = get_ports_connecting(switch, aj_switch)
            output_port = pair_port[0]
        openflow_command = build_openflow_flowmod(match=match, output_port=output_port)
        install_flowmod_on_switch_with_dpid(openflow_command, switch)
    
    # install flows on the path from middlebox to server
    for switch in path_middlebox_to_server:
        in_port = get_input_port(path_middlebox_to_server, switch)
        match = build_match_for(source_ip=source_ip, destination_ip=destination_ip, source_port=source_port, destination_port=destination_port, in_port=in_port, source_mac=source_mac, destination_mac=destination_mac)
        switch_index = path_middlebox_to_server.index(switch)
        
        if switch_index == len(path_middlebox_to_server)-1:
            output_port = 1
        else:
            aj_switch = path_middlebox_to_server[switch_index+1]
            pair_port = get_ports_connecting(switch, aj_switch)
            output_port = pair_port[0]
        openflow_command = build_openflow_flowmod(match=match, output_port=output_port)
        install_flowmod_on_switch_with_dpid(openflow_command, switch)   
